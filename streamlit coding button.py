...
if st.button("🔄 Update Data"):
    ...
    if temperature is not None and humidity is not None:
        st.metric("🌡️ Suhu", f"{temperature} °C")
        st.metric("💧 Kelembaban", f"{humidity} %")
        st.info(get_ai_keterangan(temperature, humidity))
    else:
        st.warning("⚠️ Gagal mengambil data dari Ubidots.")
else:
    st.write("Tekan tombol **🔄 Update Data** untuk menampilkan informasi suhu dan kelembapan terbaru.")
